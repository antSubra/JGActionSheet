...
{
  return [self initWithTitle:title message:message buttonTitles:buttonTitles buttonStyle:buttonStyle checkImage:nil selectedIndex:nil selectedColor:nil];
}
...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message buttonTitles:(NSArray *)buttonTitles buttonStyle:(JGActionSheetButtonStyle)buttonStyle checkImage:(UIImage *)checkImage selectedIndex:(NSNumber *)selectedIndex selectedColor:(UIColor *)color {
  ...
  if (self) {
    ...
    if (title) {
      [self createTitleWithText:title];
    }
    
    if (message) {
      [self createMessageWithText:message];
    }
    ...
    if (buttonTitles.count) {
      ...
      for (NSString *str in buttonTitles) {
        BOOL selected = self.selectedIndex && [selectedIndex isEqualToNumber:@(index)];
        JGActionSheetButtonStyle style = (selected)?JGActionSheetButtonStyleSelected:JGActionSheetButtonStyleDefault;
        JGButton *b = [self makeButtonWithTitle:str style:style];
        b.row = (NSUInteger)index;
        float titleSpacing = kInsetSpacing * 2 + ((selected)?0:self.image.size.width);
        b.contentEdgeInsets = UIEdgeInsetsMake(0, kInsetSpacing * 2., 0, 0);
        b.titleEdgeInsets = UIEdgeInsetsMake(0, titleSpacing, 0, 0);
        
        if (selected) {
          [b setBackgroundImage:[self pixelImageWithColor:self.selectedColor] forState:UIControlStateNormal];
          [b setImage:self.image forState:UIControlStateNormal];
        }
        [self addSubview:b];
        [buttons addObject:b];
        index++;
...
- (void)createTitleWithText:(NSString *)title {
...
- (void)createMessageWithText:(NSString *)message {
...
- (void)showInView:(UIView *)view animated:(BOOL)animated {
  ...
  if (!animated) {
    ...
    CGFloat duration = 0.3; //(iPad ? 0.3f : kAnimationDurationForSectionCount(self.sections.count));
